...
class Block3D(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=2.0, drop_ratio=0., attn_drop_ratio=0.):
        ...
        self.drop_path = nn.Identity()  # 简化，移除drop path
...
class MGFIM(nn.Module):
    ...
    def forward(self, enc_feature, dec_feature):
        ...
        return enhanced_feature  # 或 torch.cat([x_attended, g_attended], dim=1)
...
class EViT_ResUNet3D(nn.Module):
    def __init__(self, input_ch=1, output_ch=1, init_feats=32):
        ...
        self.svit_layer = EGEM(
            input_dim=init_feats * 16,  # 使用enc5的输出通道数
            num_heads=2,  # 极简配置：2个头
            mlp_ratio=2.0,  # 减小MLP比率
            depth=1,  # 极简配置：只有1层
            drop_ratio=0.1
        )

        self.gated_attention_layer4 = MGFIM(
            enc_channels = init_feats * 8,  # enc4的通道数
            dec_channels = init_feats * 16,  # up4的通道数 (enc5经过ViT后)
            F_int = init_feats * 4  # 中间通道数
        )
        self.gated_attention_layer3 = MGFIM(
            enc_channels = init_feats * 4,  # enc3的通道数
            dec_channels = init_feats * 8,  # dec4上采样后的通道数
            F_int=init_feats * 2  # 中间通道数
        )
        self.gated_attention_layer2 = MGFIM(
            enc_channels = init_feats * 2,  # enc2的通道数
            dec_channels = init_feats * 4,  # dec3上采样后的通道数
            F_int=init_feats  # 中间通道数
        )
        self.gated_attention_layer1 = MGFIM(
            enc_channels = init_feats,  # enc1的通道数
            dec_channels = init_feats * 2,  # dec2上采样后的通道数
            F_int=init_feats // 2  # 中间通道数
        )
